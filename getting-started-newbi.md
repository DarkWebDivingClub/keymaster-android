...
connection dies. Recovery steps:

1. **Reconnect the USB cable** between phone and laptop. ADB
   comes back automatically — verify with:

   ```bash
   adb devices
   ```

2. **Re-establish BT PAN from the phone.** Go to BT settings →
   paired laptop → tap **CONNECT**. The phone gets an IP on the
   BT PAN network.

3. **Verify BT PAN is up** from the laptop:

   ```bash
   ping -c 1 10.44.0.3
   ```

   If ping fails, see [BT PAN troubleshooting](#bt-pan-troubleshooting)
   below.

4. **KeyMaster auto-reconnects.** The app detects the Bluetooth
   network and re-attaches automatically — no QR scan needed.
   This may take up to 30 seconds (the app retries up to 5 times
   if the relay isn't immediately reachable).

   Monitor progress:

   ```bash
   adb logcat -s AvatarService -d | tail -20
   ```

   You should see:

   ```
   Network available: transport=BLUETOOTH
   Connection is down, triggering re-attach after BLUETOOTH network restored
   Re-attached to ws://10.44.0.1:7777 after network restore
   ```

5. **Verify on the desktop** that SSH and GPG work:
...
If the app shows "Reconnect failed" in the notification, the relay
was unreachable during all 5 retry attempts. Re-establish network
connectivity and re-attach manually (open app → Attach to Avatar →
scan QR).
